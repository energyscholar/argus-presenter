...
export const CONSTRUCTOR_COVERAGE = {
  ...
  sessionStoreFile:          { declined: 'DEPLOYMENT CONFIG / SECURITY (Plan 0693 T1) — where the durable OIDC session store lives, so a sign-in survives a restart. ⛔ That file is a CREDENTIAL AT REST (it holds sha256(sid) + the principal for every live session), so an agent-settable destination would be a write primitive pointed at other people\'s identities, and a readable one a route to enumerate them. Resolved by the CLI / presenter_start from the declared state dir; overridable only in presenter-config.json.', deploymentOnly: true },
  ...
  stateDir:                  { declined: 'DEPLOYMENT CONFIG (Plan 0720 RUN C, F18) — where the LIVE SESSION STATE is dumped so it survives a restart or a CI redeploy. Resolved by the CLI / presenter_start through the estate routing table\'s `persist state` row; a bare createServer() leaves it null and writes nothing, which is what keeps this suite out of a human\'s real state directory. ⚠ deploymentOnly:false is the HONEST answer today: nothing reads it from presenter-config.json, only the env chain ($PRESENTER_STATE_DIR / $PRESENTER_CAMPAIGN_DIR / $PRESENTER_DATA_DIR) and the direct option, so the 0551 C7 pairing does not apply. When a config section declares it, add it to DEPLOYMENT_ROUTED_OPTIONS and flip this flag. ⛔ Never an agent knob: an agent that could move this file could redirect — or quietly lose — the whole table\'s session.', deploymentOnly: false },
...
export const API_COVERAGE = {
  ...
  durableState:        { declined: 'PARTLY EXPOSED, DELIBERATELY, and by the same rule as `sessionLog` directly above. STATE \u2014 status(): configured / file / paths / quietMs / maxMs / writes / failures / restoredLeaves / skipped \u2014 is reported by presenter_health (Plan 0720 RUN C), because \u201cwill this session survive the push I am about to make?\u201d is a question an operator must be able to ask BEFORE the push. CONTENT is not exposed anywhere: the file holds the live board, the turn order and the damage, and a read surface for it would be a second, ungated route to the session state that permissions already govern on the socket. \u26d4 restore() and flushSync() are NOT agent knobs and must not become tools here: automatic, debounced-with-a-ceiling persistence is the mechanism (AUDIT-1 \u2014 nobody calls a capture endpoint before the power cuts, and the CI does not call one before it restarts the service), and an agent-triggered restore mid-session would overwrite live play with a snapshot. The GM/agent door for capture and restore is REST (trust:self), and it belongs with plan 0722, not ahead of it. noteOp()/close() are the server\u2019s own plumbing.' },
