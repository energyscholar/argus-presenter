...
if (typeof AudioWorkletProcessor !== 'undefined' && typeof registerProcessor === 'function') {
  ...
  class VoiceWorkletProcessor extends AudioWorkletProcessor {
    constructor(options) {
      ...
      // S206 fix: thresholds tuned for a REAL AGC-off mic (conversational RMS ~0.008–0.02), not the
      // loud 0.19-RMS test tone. Lower VAD threshold + lower noise floor so quiet speech is boosted
      // above threshold and endpoints; shorter minSpeech so brief words ("yes"/"stop") still register.
      this.vad = makeVad({ frameMs: FRAME_MS, threshold: o.vadThreshold || 0.012,
        minSilenceMs: o.minSilenceMs || 600, minSpeechMs: o.minSpeechMs || 120 });
      this.normOpts = { targetRms: o.targetRms || 0.12, noiseFloor: o.noiseFloor || 0.004,
        maxGain: o.maxGain || 8, enabled: o.normalize !== false };
    ...
    flushFrame() {
      const seg = this.acc.subarray(0, FRAME_LEN);
      const raw = rms(seg);                                   // S206 meter: input level BEFORE normalize
      const frame = normalizeConservative(seg, this.normOpts);
      const nrm = rms(frame);                                 // what the VAD actually sees vs threshold
      this.lvlRaw = Math.max(this.lvlRaw || 0, raw); this.lvlNrm = Math.max(this.lvlNrm || 0, nrm);
      if ((this.lvlN = (this.lvlN || 0) + 1) >= 75) { this.emit({ type: 'level', raw: this.lvlRaw, nrm: this.lvlNrm }); this.lvlN = 0; this.lvlRaw = 0; this.lvlNrm = 0; }   // ~1.5s window
      const ev = this.vad.push(nrm);
      if (ev === 'start') {
