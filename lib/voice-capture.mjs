 ...
 *    autoGainControl:false, and echoCancellation:false since Plan 0685 R2 — the browser echo
 *    canceller forced an in-call audio route on Android; the echo guard is duckWhilePlaying now
...
export async function startCapture({ getSocket, socket, opts = {} } = {}) {
  ...
  //
  // ⛔ echoCancellation IS OFF ON PURPOSE — Plan 0685 R2, and it is a MITIGATION WHOSE CAUSE IS
  // STILL UNKNOWN. It does NOT explain plan 0682 and must not be written up as one.
  //   MEASURED: asking for echoCancellation puts Chrome-on-Android into COMMUNICATION mode, which
  //   routes playback down the in-call path (earpiece on the phone speaker, SCO on Bluetooth). The
  //   operator had to hold the phone to his ear to hear anything, and with the mic open even an
  //   unrelated video played very softly. Flag off: loud and usable. That is the whole evidence.
  //   NOT EXPLAINED: why it started. The flag is ORIGINAL code (c7e2670); every client audio file
  //   is byte-identical to the release of a week earlier, when it worked; and the operator is
  //   explicit that nothing changed on his phone. Anything requiring his device to have changed
  //   contradicts the primary evidence. ⛔ Plan 0682 stays OPEN.
  // The echo this flag defended against is handled capture-side by duckWhilePlaying (R1, below).
  // ⛔ ORDER: this flag may only be off while that guard is wired. Removing the duck re-arms it.
  let stream;
  try {
  ...
  const ctrl = {
    ...
    duckedSegs: 0, duckedPcm: 0,          // Plan 0685 R1 (G6): what the echo guard discarded, counted
    badgeEl: null,
  };
  ...
  node.port.onmessage = (e) => {
    const d = e.data; if (!d || ctrl.stopped) return;   // S206: nothing is gated on the duck HERE — see seg-start and pcm below
    if (d.type === 'level') { trace('L level', { raw: +Number(d.raw).toFixed(4), nrm: +Number(d.nrm).toFixed(4) }); return; }   // S206 meter
    if (d.type === 'seg-start') {
      // Plan 0685 R1 — ⭐ DUCKING WINS OVER BARGE-IN, stated rather than left emergent. The server
      // treats a segment opened during playback as an interruption (maybeBargeIn at seg-start).
      // With echoCancellation OFF (R2) the mic hears the agent's own TTS at full level, so such a
      // segment is at least as likely to be the agent as the user: acting on it would let the agent
      // interrupt ITSELF and reopen the loop this guard closes. So while ducked we open NO segment.
      // Barge-in resumes the instant the duck lifts, and utterances are short (~300 chars clamped),
      // so the blind window is seconds. ⛔ Do not "fix" this by letting the bracket through.
      // G6: the discard is COUNTED and traced, never silent. Rationale + proof: test/live/V0685-*.
      if (ctrl.ducked) { ctrl.duckedSegs++; trace('S2 seg-start SUPPRESSED (ducked)', { seq: d.seq, duckedSegs: ctrl.duckedSegs }); return; }
      ctrl.curSeq = d.seq; ctrl.segOpen = true; ctrl.sent = 0; ctrl.dropped = 0; sendJson({ t: 'voice_seg_start', seq: d.seq }); trace('S2 seg-start', { seq: d.seq, rs: (resolveSocket() || {}).readyState });
    }
    else if (d.type === 'pcm') {
      if (ctrl.ducked) { ctrl.duckedPcm++; return; }   // drop audio while ducked (counted, G6); an already-open bracket still closes
  ...
  // RT-5 echo guard. Plan 0685 R1 wired it at last — app/presenter.html drives it from the
  // speechSynthesis lifecycle (start -> true; end/error/cancel/watchdog -> false). Until then it
  // was defined here and called from NOWHERE, so the mitigation had never once run.
  // ⛔ CAPTURE-SIDE ONLY: one boolean, no stream stop, no track release, no getUserMedia.
  ctrl.duckWhilePlaying = (on) => { ctrl.ducked = !!on; };
