...
export async function startCapture({ getSocket, socket, opts = {} } = {}) {
  ...
  node.port.onmessage = (e) => {
    ...
    if (d.type === 'seg-start') {
      // Plan 0685 R1 — ⭐ DUCKING WINS OVER BARGE-IN. Stated here rather than left emergent.
      // The server treats a segment opened during agent playback as an interruption (maybeBargeIn
      // at seg-start, app/server.mjs). With echoCancellation OFF (R2) the microphone hears the
      // agent's own TTS at full level, so a segment opened while ducked is at least as likely to be
      // the agent as the user — acting on it would let the agent interrupt ITSELF and would reopen
      // the exact feedback loop this guard exists to close. So while ducked we open NO segment:
      // barge-in is deliberately suppressed for the duration of playback, and resumes the instant
      // the duck lifts. Utterances are short (the server clamps a spoken précis to ~300 chars), so
      // the blind window is seconds. ⛔ Do not "fix" this by letting the bracket through.
      // G6: the discard is COUNTED and traced, never silent.
  ...
  // RT-5 echo guard. Plan 0685 R1 wired this at last: app/presenter.html calls it from the
  ...
  // ⛔ CAPTURE-SIDE ONLY, and it must stay that way: one boolean, no stream stop, no track release,
  // no getUserMedia. Hands-free operation is a requirement, not a preference.
