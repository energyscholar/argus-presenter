...
/**
 * Where this deployment's durable state lives, and WHICH RUNG SAID SO.
 * → { stateDir, stateDirSource, campaign } — see the four rungs documented above.
...
export function resolveStateDir({ env = process.env } = {}) {
  ...
  if (named) return { stateDir: resolve(named), stateDirSource: STATE_DIR_ENV, campaign: null };
  const campaignDir = trimmed(env[CAMPAIGN_DIR_ENV]);
  if (campaignDir) return { stateDir: resolve(campaignDir, 'state'), stateDirSource: CAMPAIGN_DIR_ENV, campaign: null };
...
export function flattenSubtree(root, prefix, out = {}, isEphemeralPath = null) {
  ...
  for (const k of keys) {
    ...
    if (eph(prefix + '/' + k)) continue;                      // omission 3
    emitted++;
