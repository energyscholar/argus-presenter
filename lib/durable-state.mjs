 ...
 *
 * F2d (0738, R-241) adds `gm`: undetected salvos live at `gm/combat/salvos/...` and must survive a
 * restart same as the board does. No caller in this checkout sets `$PRESENTER_STATE_PATHS` or
 * passes `statePaths` to `createServer()` (measured at F2d's compile: every call site in this
 * repo omits both), so THIS constant is the
 * effective deployed value, not merely a fallback; wiring it dynamically from every installed
 * plugin's manifest reaches `app/server.mjs`, which is NOT in this run's files (a later run's
 * boundary, not a silent narrowing — see the plan's own RISKS). Until then this is the HAND-COMPUTED join of the
 * pre-F2d value (`['shared', 'ships']`) and this deployment's one installed plugin's declared
 * prefixes (`systems/traveller/plugins/starship-ops/plugin.json`'s `durableStatePaths`) — see
 * `joinStatePaths()` below, and `t0738-02` (rest-resources.test.mjs) which proves the two agree.
 */
export const DEFAULT_STATE_PATHS = Object.freeze(['shared', 'ships', 'gm']);
...
/**
 * F2d (0738, R-241). Pure set-union, first-occurrence order preserved: the durable prefixes a
 * deployment actually persists are the engine's own base plus every prefix any plugin declares.
 * Exported so a future caller that wires plugin manifests into `DEFAULT_STATE_PATHS` at boot
 * (reaching `app/server.mjs` — not this run's file) can reuse it rather than re-deriving it; THIS
 * run's own use of it is only to PROVE, in `rest-resources.test.mjs`'s `t0738-02`, that the literal
 * above agrees with what this function computes.
 */
export function joinStatePaths(...lists) {
  const out = [];
