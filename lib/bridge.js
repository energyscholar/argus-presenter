...
(function (global) {
  ...
  function send(msg) {
    ...
    if (embedded) {
      ...
      /* ⛔⛔ IT REPORTS WHETHER IT DELIVERED. This used to `return` bare on success and fall
         through on a thrown postMessage, so every caller — `answer`, `ready`, `debug` — learnt
         nothing about whether the frame left. `debug()` in particular returned `true` having
         transmitted NOTHING, which is the exact failure it was written to make impossible.
         ⚠ Callers that ignore the value are unaffected: `return true`/`false` is strictly more
         information than `return undefined`. */
      try { global.parent.postMessage(msg, '*'); return true; } catch (e) { /* fall through */ }
    }
    try { global.dispatchEvent(new CustomEvent(NS + ':message', { detail: msg })); return true; }
  ...
  var Argus = {
    ...
    /*
     * ⭐⭐⭐ NETWORKED DEBUG IS A FIRST-CLASS CHANNEL, not something each component invents.
     *
     * ⛔⛔ WHY THIS EXISTS: a desk reporter written on 2026-09-09 called `Argus.result(...)`, which
     * has never been a method here. It was wrapped in a try/catch — as a diagnostic should be — so
     * it threw on every call, silently, and reported ZERO frames while looking like it worked. A
     * debug channel that fails quietly is worse than none: it makes the absence of evidence look
     * like the absence of a problem.
     *
     * ⇒ `where` names the CODE LOCATION and `data` carries the values that decided. Emitting is
     * best-effort by design (a diagnostic must never be why a component dies) — but a failure to
     * emit is itself recorded on `window.__apDebugFailed`, so "no frames arrived" can always be
     * told apart from "nothing happened".
     */
    debug: function (where, data) {
      try {
        /* ⛔⛔ `send()` SWALLOWS A FAILED postMessage AND FALLS THROUGH, so this used to return
           `true` having transmitted NOTHING. `__apDebugFailed` caught a DEAD REPORTER and not a
           DROPPED FRAME — two different silences, and only one was covered. A reporter that
           reports success it cannot verify is the bug it exists to prevent, in itself. */
        var delivered = send({ type: 'debug', where: String(where || '?'),
               value: data === undefined ? null : data,
               at: Math.round((global.performance && global.performance.now()) || 0) });
        if (!delivered) {
          /* ⛔ A DROPPED FRAME AND A DEAD REPORTER ARE TWO DIFFERENT SILENCES, and only the second
             was covered. Both land here now, distinguishable by `dropped`. */
          try {
            (global.__apDebugFailed || (global.__apDebugFailed = [])).push({
              where: String(where || '?'), dropped: true, err: 'send() reported no delivery' });
          } catch (e2) { /* nothing left that can report */ }
        }
        return !!delivered;
      } catch (e) {
        try {
          (global.__apDebugFailed || (global.__apDebugFailed = [])).push({
            where: String(where || '?'), err: String((e && e.message) || e) });
        } catch (e2) { /* nothing left that can report */ }
        return false;
      }
    },
