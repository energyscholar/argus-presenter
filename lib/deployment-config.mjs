 ...
 */
export const ROOM_KEYS = Object.freeze(['port', 'bindHosts', 'plugins', 'record', 'voice', 'label']);
...
export function roomDefaults() {
  return { port: null, bindHosts: null, plugins: [], record: 'none', voice: false, label: null };
}
...
export function normalizeRoomConfig(room, where = '(built-in defaults)', roomName = '(defaultRoom)') {
  ...
  return { ...src, port, bindHosts, plugins, record, voice, label };
}
...
export function resolveRoom(env = process.env, config = {}) {
  ...
  if (!wanted) {
    return {
      name: null,
      room: normalizeRoomConfig(cfg.defaultRoom, where, '(defaultRoom)'),
      source: 'default',
      sources: roomValueSources(cfg.defaultRoom),
  ...
  return {
    ...
    room: rooms[wanted],
    source: 'env',
    sources: roomValueSources(cfg.rooms[wanted]),
    configPath: cfg.configPath || null,
    configSource: cfg.configSource || null,
  };
}
...
export function roomStartupLine(resolved = {}) {
  ...
  return `room: ${name}`
    + ` · port ${r.port === null ? 'unset' : r.port}${from('port')}`
    + ` · bindHosts ${binds}${from('bindHosts')}`
    + ` · plugins ${plugins}${from('plugins')}`
    + ` · record ${r.record}${from('record')}`
    + ` · voice ${r.voice}${from('voice')}`
    + ` · from ${where}`
