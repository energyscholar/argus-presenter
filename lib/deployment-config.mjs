...
/* ═══════════════════════════════════════════════════════════════════════════════════════════════
 * Plan 0693 T3 — THE CONTROL TOKEN IS A PINNED CREDENTIAL, NOT A PER-BOOT ACCIDENT.
 *
 * WHAT WAS WRONG. When no credential was configured, `presenter_start` MINTED one
 * (`randomBytes(16)`) so the module write-back could never ship open — correct, and it had an
 * unnoticed cost: the token changed on EVERY restart, and jill restarts on every push. The
 * operator's one remaining way into his own box (the OIDC session having been destroyed by the same
 * restart — see T1) therefore expired silently whenever anybody deployed.
 *
 * ⇒ A deployment may DECLARE the token, in its own file or in the environment, and it then survives
 * every restart unchanged. ⛔ THE MINT-WHEN-ABSENT FALLBACK STAYS: it is what stops an
 * unconfigured deployment shipping an open module write-back, and removing it would trade a stale
 * credential for no credential.
 * ⛔ AND A CONFIGURED TOKEN IS NEVER PRINTED. A minted one is returned to its caller because a
 * secret nobody can read is the same as no write surface at all; a token the operator already holds
 * has no such excuse, and printing it puts a live credential in the journal.
 * ═══════════════════════════════════════════════════════════════════════════════════════════════ */

/** The env override, checked before the config file (SCREAMING_SNAKE per docs/naming-canon.md §4). */
export const CONTROL_TOKEN_ENV = 'PRESENTER_CONTROL_TOKEN';
...
/**
 * The DECLARED control token for this deployment, or null. Order: $PRESENTER_CONTROL_TOKEN >
 * config `controlToken` > null (⇒ the caller mints one, as before).
 * Returns { controlToken, controlTokenSource } so a caller can say WHERE it came from without
 * saying WHAT it is.
 */
export function controlTokenConfig(opts = {}) {
  const fromEnv = trimmed((opts.env || process.env)[CONTROL_TOKEN_ENV]);
  if (fromEnv) return { controlToken: fromEnv, controlTokenSource: 'env' };
  let cfg = null;
  try { cfg = loadDeploymentConfig(opts); } catch (e) { cfg = null; }
  const fromCfg = cfg ? trimmed(cfg[CONTROL_TOKEN_KEY]) : null;
  if (fromCfg) return { controlToken: fromCfg, controlTokenSource: 'config' };
  return { controlToken: null, controlTokenSource: null };
}
 ...
 */
export const IDENTITY_KEYS = Object.freeze(['oidc', 'allowlist', 'tailscale', 'breakGlass', 'revokedNonceFile', 'sessionStoreFile']);
...
/**
 * Plan 0693 T1 — validate the durable OIDC-session store path. Absent ⇒ null (the launch paths
 * derive one in the declared state dir). ⛔ This file holds sha256(sid) + the principal for every
 * live sign-in, so a deployment that states its own path is stating where a CREDENTIAL AT REST
 * lives: it must be outside any repository and outside any backup (0696 F9).
 */
export function normalizeSessionStoreFileConfig(sessionStoreFile, where = '(built-in defaults)') {
  if (sessionStoreFile === undefined || sessionStoreFile === null) return null;
  if (!nonEmptyString(sessionStoreFile)) {
    throw new IdentityConfigError(`sessionStoreFile in ${where} must be a non-empty path string (or absent), got ${JSON.stringify(sessionStoreFile)}`);
  }
  return sessionStoreFile.trim();
}
...
const IDENTITY_NORMALIZERS = Object.freeze({
  ...
  sessionStoreFile: normalizeSessionStoreFileConfig,
});
 ...
 *
 * Returns every IDENTITY_KEY plus `configPath`/`configSource`, so a caller can say WHICH file won —
 ...
 */
export const ROOM_KEYS = Object.freeze(['port', 'bindHosts', 'plugins', 'record', 'voice', 'label', 'profile', 'pluginsDir', 'transcriptDir', 'mcpPort']);
...
export function roomDefaults() {
  return { port: null, bindHosts: null, plugins: [], record: 'none', voice: false, label: null, profile: null, pluginsDir: null, transcriptDir: null, mcpPort: null };
}
...
export function normalizeRoomConfig(room, where = '(built-in defaults)', roomName = '(defaultRoom)') {
  ...
  /* mcpPort — ⛔ THE ROOM'S MCP CONTROL PORT. `PRESENTER_MCP_HTTP` is process-global, so two room
   * processes started from one unit template both bind it: the second dies, or the tools address
   * whichever won and an operator drives the wrong room believing they are driving this one. So
   * the port is a property OF THE ROOM.
   *
   * 0 is refused here for the same reason `port` refuses it — a control port is an address other
   * things must be able to name — even though the env fallback still reads 0 as "no HTTP MCP",
   * which is that variable's existing meaning and is preserved. */
  let mcpPort = d.mcpPort;
  if (src.mcpPort !== undefined && src.mcpPort !== null) {
    if (!Number.isInteger(src.mcpPort) || src.mcpPort < 1 || src.mcpPort > 65535) {
      throw new RoomConfigError(`${at} has mcpPort ${JSON.stringify(src.mcpPort)} — must be a whole number 1..65535 (0 is refused: an MCP control port is an address a client must be able to name)`);
    }
    mcpPort = src.mcpPort;
  }
  /* ⛔ AND IT MAY NOT BE THIS ROOM'S OWN HTTP PORT. One process, one bind: the second listener
   * fails with EADDRINUSE at a moment nothing is watching, and which of the two came up first is
   * a race. Caught here rather than at bind time, where the answer is nondeterministic. */
  if (mcpPort !== null && port !== null && mcpPort === port) {
    throw new RoomConfigError(`${at} declares mcpPort ${mcpPort} AND port ${port} — one process cannot bind one port twice. Give the MCP control port its own number.`);
  }
  ...
  return { ...src, port, bindHosts, plugins, record, voice, label, profile, pluginsDir, transcriptDir, mcpPort };
}
...
export const CAPABILITY_ENV = Object.freeze({
  ...
  mcpPort: 'PRESENTER_MCP_HTTP',
});

/** Every key `resolveRoomCapabilities` answers for, in startup-line order. */
export const CAPABILITY_KEYS = Object.freeze(['profile', 'voice', 'record', 'pluginsDir', 'transcriptDir', 'mcpPort']);
...
export function parseCapabilityEnv(key, raw) {
  ...
  if (key === 'mcpPort') {
    /* ⚠ `0` and unset BOTH mean "no HTTP MCP" — that is `PRESENTER_MCP_HTTP`'s existing meaning
     * (`Number(process.env.PRESENTER_MCP_HTTP || 0)`, then `if (HTTP_PORT > 0)`), and this layer
     * preserves it rather than inventing a second interpretation of a variable already in use on
     * the live box. ⛔ Anything that is not a port number still throws: `Number('31OO')` is NaN,
     * and NaN > 0 is false, so today a typo silently disables the control port. */
    if (v === '0') return undefined;
    const n = Number(v);
    if (!Number.isInteger(n) || n < 1 || n > 65535) {
      throw new RoomConfigError(`${name}=${JSON.stringify(raw)} is not a port number — must be a whole number 1..65535, or 0/unset for "no HTTP MCP". ⛔ REFUSING to coerce: Number() answers NaN to a typo and NaN > 0 is false, so the control port would simply not open and nothing would say why.`);
    }
    return n;
  }
...
export function normalizeRoomsConfig(rooms, where = '(built-in defaults)') {
  ...
  const out = {};
  /* ⛔⛔ Plan 0684 R5 — NO TWO ROOMS MAY CLAIM ONE MCP CONTROL PORT, and the answer is a NAMED
   * STARTUP ERROR rather than last-writer-wins. Two processes racing for one port do not produce a
   * wrong VALUE that a human can read back; they produce a nondeterministic outcome — one unit
   * dead in a way that looks like a crash, or an operator's tools quietly attached to the other
   * room. That is only visible from HERE, where every declared room is in one place.
   * ⚠ SCOPE: this sees `rooms{}`. `defaultRoom` is not compared against it, because a process that
   * resolves to defaultRoom is by definition the single-room legacy case with nothing to collide
   * with. */
  ...
  for (const [rawName, v] of Object.entries(rooms)) {
    ...
    const mp = out[name].mcpPort;
    if (mp !== null && mp !== undefined) {
      if (claimed.has(mp)) {
        throw new RoomConfigError(
          `rooms in ${where}: "${claimed.get(mp)}" and "${name}" BOTH declare mcpPort ${mp}. ` +
          `Two room processes cannot bind one MCP control port — the second fails at a moment nothing ` +
          `is watching, or the tools attach to whichever won and an operator drives the wrong room ` +
          `believing they are driving this one. REFUSING to let the last one written win. Give each ` +
          `room its own MCP port.`);
      }
      claimed.set(mp, name);
    }
  }
...
export function roomStartupLine(resolved = {}) {
  ...
  return `room: ${name}`
    ...
    + ` · mcpPort ${unset(val('mcpPort'))}${from('mcpPort')}`
    + ` · from ${where}`
