...
import { withFixturePlugins, fixturePluginDir } from '../fixtures/plugins.mjs';
...
test('A1 — city-grid renders ONLY via the fixture preset', async () => {
  const r = await withFixturePlugins(() => drive({
...
test('A1 — core map source contains no domain art; the plugin does', () => {
  ...
  const pluginSrc = readFileSync(join(fixturePluginDir('example'), 'map-presets.js'), 'utf8');
