...
test('G1 — a gm/ slice diff is broadcast to the presenter, never to a player', async () => {
  ...
  try {
    ...
    gm.ws.send(JSON.stringify({ t: 'op', path: 'public/note', verb: 'set', value: 'hello', opId: 'g2' }));
    await wait(250);
    ...
    expect(pStr.includes('public/note'), 'player DOES receive the public diff', pStr);
...
test('G1 — a late player\'s snapshot omits the gm/ slice; presenter\'s includes it', async () => {
  ...
  try {
    ...
    server.store.apply({ path: 'public/x', verb: 'set', value: 1 }, { userId: 'gm', role: 'presenter' });
    ...
    expect(pSnap && pSnap.state.public && pSnap.state.public.x === 1, 'player snapshot keeps public state');
