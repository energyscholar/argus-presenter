...
export function createServer({ port = 0, controlToken = null, rolePassword = null, roleSeed = null, voiceEnabled = undefined, capSecret = null, profile = DEFAULT_PROFILE, settlingMs = null, queueMaxPending = null, queueTtlMs = null, perTurnBudgetMs = null, perTurnWrapMs = null, floorThresholds = null, sessionLogDir = null, enforceOAuth = undefined, allowPasswordCommandOnLAN = undefined, allowlist = null, oidc = null, oidcDeps = null, oidcSessionTtlMs = null, tailscale = null, tailscaleResolve = null, tailscaleWhois = null, breakGlass = null, breakGlassDeps = null, revokedNonceFile = null, bindHosts = null } = {}) {
  ...
  function targets(target) {
    ...
    return socketsFor(target);   // by userId — ALL of that person's live sockets (A4 fan-out)
      ...
      /* ── WIRE ACTIONS — the message chain, converted to a table (Plan 0661 phase 1) ─────────
       * ⭐ Registered HERE, inside createServer, so every closure the bodies use (send, emit,
       *   handleOp, handleControl, voiceSegFinalize, unpeekTo, log, telem…) resolves exactly as
       *   it did in the chain. Handlers take only { m, c, ws } — no context object has to
       *   enumerate the server's internals, and nothing is captured at module scope.
       * ⭐ Bodies are VERBATIM, extracted by AST node range — a `return` inside one exits the
       *   handler, which is what it did to the message handler: the dispatch returns either way.
       */
