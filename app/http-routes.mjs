...
export function createHttpHandler(ctx) {
  /* ── THE ROUTE TABLE (Plan 0661 phase 1b) ────────────────────────────────────────────────
   * The 27-branch `if (req.url === …)` chain, declared instead of walked. Bodies are VERBATIM,
   * lifted by AST node range; each handler NAMES the ctx bindings it uses, so a route's
   * dependencies are readable without tracing a 400-line closure.
   *
   * ⛔ THREE TABLES, NOT ONE — the chain tested three different things and collapsing them would
   *   change behaviour:
   *     exactRoutes  — matched the FULL url. `/branch.mjs?v=1` did NOT match `/branch.mjs` and
   *                    still must not; keying these by path-before-query would silently start
   *                    serving them.
   *     pathRoutes   — the `=== '/x' || startsWith('/x?')` idiom: path, query tolerated.
   *     prefixRoutes — ORDERED, checked last: `/api/modules/`, `/api/module-admin/`, …
   *
   * ⭐ Order is not load-bearing, and that is MEASURED, not assumed: no route was shadowed by an
   *   earlier one (checked pairwise, every prefix against every later key). That is what makes a
   *   table equivalent to a chain. A future shadowed route would resolve differently here than it
   *   did in the chain, so that check belongs in the phase-2 lint, not in this comment.
   *
   * ⭐ Built ONCE per server. The bindings are read PER REQUEST into `S` — see `ctx.api`'s temporal
   *   dead zone in the file header; hoisting the read would break it.
   */
  const exactRoutes = new Map();
  const pathRoutes = new Map();
  const prefixRoutes = [];

  pathRoutes.set("/auth/login", ({ req, res, oidcAuth }) => {
  ...
  prefixRoutes.push(["/auth/callback", ({ req, res, api, CONTROL, oidcAuth }) => {
  ...
  pathRoutes.set("/auth/break-glass", ({ req, res, breakGlassAuth }) => {
  ...
  pathRoutes.set("/auth/logout", ({ req, res, oidcAuth }) => {
  ...
  pathRoutes.set("/", ({ req, res, api, htmlHeaders, renderPresenterPage, VOICE_ENABLED }) => {
        // Plan 0473 P0: strip the voice block(s) unless voice is enabled ⇒ zero voice bytes when off.
        res.writeHead(200, htmlHeaders());
        /* ⭐⭐ AVAILABILITY IS A SERVER FACT; PERMISSION IS A CLIENT REVEAL. Rendering the mic from the
         *   requester's session made LOAD ORDER decide the UI: sign in after the page loaded — which is
         *   exactly what the sign-in flow does, since it lands you on /control — and you got no mic
         *   until a reload. Measured on a real sign-in.
         *   ⇒ ship the blocks whenever the server has voice, and let the page REVEAL the row from
         *     GET /api/auth-state, which it already re-reads. That is the same pattern the sign-in row
         *     itself uses ("ships hidden; revealed only when auth-state reports…").
         *   ⛔ Costs nothing in security: the gate is `voice_seg_start` on the wire, and always was. */
        res.end(renderPresenterPage(VOICE_ENABLED));
  });

  pathRoutes.set("/diag", ({ req, res, api, CONTROL, htmlHeaders }) => {
  ...
  pathRoutes.set("/control", ({ req, res, CONTROL, htmlHeaders }) => {
        res.writeHead(200, htmlHeaders());
        res.end(readFileSync(CONTROL, 'utf8'));
  });

  pathRoutes.set("/manage", ({ req, res, __dirname, htmlHeaders }) => {
  ...
  pathRoutes.set("/creator", ({ req, res, __dirname, htmlHeaders }) => {
  ...
  exactRoutes.set("/branch.mjs", ({ req, res, __dirname, sendStatic }) => {
  ...
  exactRoutes.set("/validate.mjs", ({ req, res, __dirname, sendStatic }) => {
  ...
  exactRoutes.set("/lib/breakdown.js", ({ req, res, LIB, sendStatic }) => {
  ...
  pathRoutes.set("/api/auth-state", ({ req, res, api, authState }) => {
        /*
  ...
  pathRoutes.set("/api/modules", ({ req, res, api, catalogueReadAuthed, CONTROL, listModules }) => {
  ...
  pathRoutes.set("/api/module-admin", ({ req, res, ARCHIVE_DIR, listModulesAdmin, MODULE_STATUSES, MODULES_DIR, moduleWriteAuthed }) => {
  ...
  prefixRoutes.push(["/api/module-admin/", ({ req, res, api, moduleAdminOp, moduleWriteAuthed }) => {
  ...
  prefixRoutes.push(["/api/modules/", ({ req, res, api, catalogueReadAuthed, CONTROL, CONTROL_TOKEN, listModules, moduleCache, MODULES_DIR, moduleWriteAuthed, readModuleFile, ROLE_HASH }) => {
  ...
  pathRoutes.set("/api/session-log", ({ req, res, api, CONTROL_TOKEN, moduleWriteAuthed, sessionLog, sessionLogReadAuthed }) => {
        /*
  ...
  pathRoutes.set("/api/series", ({ req, res, api, catalogueReadAuthed, CONTROL, listSeries }) => {
  ...
  prefixRoutes.push(["/api/series/", ({ req, res, api, catalogueReadAuthed, CONTROL, listModules, moduleSummary, readSeriesFile }) => {
  ...
  pathRoutes.set("/api/situation", ({ req, res, api, httpControlAuthed, pvsConsumerKey }) => {
  ...
  pathRoutes.set("/api/work", ({ req, res, api, httpControlAuthed }) => {
  ...
  return function httpRequestHandler(req, res) {
    ...
    /* Per REQUEST: `ctx` is spread here, not at wiring time — `ctx.api` is a getter still in its
     * TDZ when this handler is constructed. One evaluation, then most-specific first. */
    const S = { req, res, ...ctx };
    const qi = req.url.indexOf('?');
    const urlPath = qi === -1 ? req.url : req.url.slice(0, qi);
    const route = exactRoutes.get(req.url) || pathRoutes.get(urlPath) ||
      (prefixRoutes.find(([p]) => req.url.startsWith(p)) || [])[1];
    if (route) { route(S); return; }
